...
const initializeConnection = ev => {
    ...
    connection.onmessage = e => {
        ...
        if (data.type === 'login' || data.type === 'disconnect') {
            user_html = data.users.map(user => `<li>${user}</li>`).join('');
            document.getElementById('users-list').innerHTML = user_html;
        }
        if (data.type === 'chat') {
            rl_class = data.username === username ? 'right' : 'left';
            sender = data.username === username ? 'You' : data.username
            chat_html = `<div class=${rl_class}>
